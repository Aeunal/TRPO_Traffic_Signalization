...
class DeeplightAgent(object):
    """
    This class is responsible for sampling from environment and making train sets
    """
    def __init__(self, env, num_phases, num_actions, path_set):
        ...
        self.scaler = scaler(num_phases + 1) #TODO observation_space.shape[0] = num_phases
    ...
    def _run_episode(self):
        ...
        for _ in range(self.timestep_limit):
            ...
            obs, reward, done, _ = self.agentStep(action)
    ...
    def agentStep(self, action_pred):
        #TODO
        # action_pred = np.squeeze(action_pred, axis=0)
        reward, action = self.env.take_action(action_pred)
    ...
    def _run_policy(self, batch_size=None):
