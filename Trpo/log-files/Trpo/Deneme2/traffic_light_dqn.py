...
from utils import log_batch_stats
...
class TrafficLightDQN:
    ...
    def __init__(self, memo, f_prefix, sumo_cmd_str):
        ...
        self.env = SumoAgent(sumo_cmd_str, self.path_set)

        self.agent = self.DIC_AGENTS[self.para_set.MODEL_NAME](self.env,
                                                                num_phases=2,
                                                                num_actions=2,
    ...
    def train(self, sumo_cmd_str, if_pretrain, use_average):
        ...
        while current_time < total_run_cnt:
            ...
            observes, actions, advantages, disc_sum_rew = self.agent.build_train_set()
            log_batch_stats(observes, actions, advantages, disc_sum_rew, self.agent.logger, current_time)
            
            # get state
            # state = self.agent.get_state(state, current_time)

            self.agent.update_policy(observes, actions, advantages)
            self.agent.update_baseline(observes, disc_sum_rew)
            self.agent.logger.write(display=True)
            ...
            memory_str = 'time = %d\taction = %d\tcurrent_phase = %d\tnext_phase = %d\treward = %f' \
                         '\t%s' \
                         % (current_time, actions[0][0],
                            observes,#.cur_phase[0][0],
                            advantages,#state,#.next_phase[0][0],
                            disc_sum_rew, repr(actions))
            print(memory_str)
